...
- Each layer MUST depend **only on the layer directly beneath it** (closed-layer
  default). If a relaxed/open-layer policy is adopted (a layer may call any
  lower layer), it MUST be recorded as a deliberate decision; even then, a layer
  MUST NOT call a layer **above** it.
...
## Accept the trade-off explicitly (do not pretend to be a sibling)
