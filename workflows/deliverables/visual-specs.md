...
| `timeline` | `milestones` (3 to 6), `now` (index of the present), `missing` (index of a milestone that has not happened: drawn hollow, reached by a dashed segment), `marker` (an annotation under the present, or under the missing milestone when `missing` is set), `caption` | A horizontal line with milestone dots; labels alternate above and below |
| `two-column` | `left`, `right` (headers), `rows` (`left cell / right cell` per item; falls back to body bullets prefixed with the header text), `prefer` (`left` or `right`), `verdict`, `icons` (`left icon; right icon`), `hues` (`main; support` per side, values `main`, `support`, or `spark`; default: the preferred side in main, the other muted) | Two cards with parallel rows; the preferred side carries check marks |
...
| `stat` | `stats` (`value / caption` per item, at most 3), `label` (small text under every caption, for example `target`), `highlight` (index drawn in the support hue; the rest in the main hue) | Cards with the value at stat-callout size; no icons |
| `panels` | `items` (`name / one-line detail` per item, at most 4), `icons` (one per item), `highlight` (index whose badge is drawn in the support hue; the rest in the main hue) | Cards in a row, each with an icon badge (or its number); a name that opens with a number (`3 specs / affected by the change`) renders as a count, the figure large with the noun beneath |
...
| `none` | `owners`, `dates`, `icons` (one per step) and `ask` (the band text: the decision in the room's terms; without it the band repeats the title and the renderer warns), read by `ask-next-steps`; `ask` (read by `agenda`); `icon` (read by `title`, `section-divider`, and `statement` for the hero marker) | Nothing; the pattern's own typography is the visual (title, quote, stat-callout, ask-next-steps, appendix-sources) |
...
- Icons name their part (slide-patterns.yml). The one decorative icon is
  the hero marker on the end slides (`title`, `section-divider`,
  `statement`): `icon:` on a `kind: none` visual, one icon for the deck's
  subject, the same wherever the marker appears.
