...
(`deck`; `memo` and a mapping's `default_kind: one-pager` resolve to `deck`
for now), look, and constraints (brand, confidentiality, must-include,
...
audience and no decision sought is not authored; that is a framing gap, not a
default.

## STEP 2 — Scope and inventory

Breadth is a declared choice, never an accident of which sections a mapping
...
authority-floor headings may be folded into a group but never dropped
silently. Record the groups under `## Story` as a Concept coverage table:
...
the artifact types it knows, as a floor, not a ceiling. Read every source
section you project. Content the deliverable needs that no artifact holds
(`gaps_to_ask`) becomes a question or a recorded assumption, never an
invented fact.

## STEP 3 — Storyboard

Choose the flow by occasion from `deck-flows.yml` (`investor-pitch`,
`evaluation-briefing`, `executive-status`, `proposal`,
...
- Distill. Write the one-sentence takeaway the audience should leave with;
  ...
  record, not a message. A concept group no message carries is `omitted` in
  the coverage table, with a reason; silent omission is the failure this step
  exists to prevent.
- Title every beat. One claim title per beat in the flow, each mapped to a
  message, with a one-line note of the exhibit that will prove it. Optional
  ...
  `keep_when_short` order when the slot is tight; never compress two beats
  into one slide.
...
- Headline pass. Detect with a prose tool that has a headline target, when
  ...
  what something does rather than what it is not. Read the titles aloud in
...
Give each unit one pattern from the beat's allowed set in `deck-flows.yml`,
chosen by content shape from `slide-patterns.yml`, honoring that file's
...
pattern's limits is two units, not a denser slide. Vary the page: a run of
claim-evidence slides alternates the visual's side; a relationship (a stack,
a loop, a hub) is a `diagram`, not a bullet list; a pivot in the argument may
...
its series, and its source>`, never "chart"), speaker notes carrying the
...
slide's words. Subset the sources when the audience needs less; never
contradict them. No HELIX vocabulary in bodies: artifact IDs, activity
...
to step 3, 5, or 7, never to "good enough":
