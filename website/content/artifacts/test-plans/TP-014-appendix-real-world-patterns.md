...
| 4 | `evolve` (thread a change) | ~24 / 8% | `"We're renaming 'phases' to activities. Make a comprehensive plan."`; `"Drop the named providers, they will cause us problems."`; `"reframe this request completely in terms of vale and make it dependent on it."` | Once (`/helix-evolve`) | §Evolve — walk graph both ways; updates in authority order; surface conflicts via §Align handoff | Extends TP-014 Scenario 3 — add `change-request-rename.txt` modeling vocabulary-rename evolve, distinct from substitution |
