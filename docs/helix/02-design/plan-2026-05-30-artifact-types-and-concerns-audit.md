...
**Track B — Mechanical contract enforcement** (no ADR dependency):
...
Goal: with the catalog contract in place and the ADRs landed, codify the boundaries the audit identified through ownership tables, phase READMEs, and prose trims. With the prior phases complete, this is the "edit phase" — no methodology debate, just translations from the ADRs and contract into the concern + artifact tree.
