...
Generated release packages may include a skill-local `references/`
directory so runtimes that install only the skill subtree remain
...
Package-local `references/` trees are generated byte-for-byte from
`workflows/` during release packaging and are validated against that
source.
...
the same agentskills.io-compliant routing skill. A generated package
filesystem-copy path remains available for older Codex CLI versions and
...
`references/` in generated packages, while source checkouts resolve the
same content from `workflows/`.

The cross-runtime install helper `npx skills add` (from Vercel Labs)
can work for runtimes that load skills from local filesystem paths only
when the installer preserves HELIX's catalog layout: either the full
source checkout with `workflows/` reachable from `skills/helix/`, or a
generated package with `skills/helix/references/`. It does NOT install
to Genie's `/Workspace/.assistant/skills/` (use
`scripts/install-genie.py` instead) and is not the canonical path for
Copilot (`.github/copilot-instructions.md` commit) or DDx (`ddx
install helix`).
...
- The artifact catalog's source lives in `workflows/` and nowhere else.
  Generated package floors under `references/` are build output and must
  not be edited or checked in as normative content. Per-runtime guides
  may describe the directory layout (so users know what they are
