...
4. **Hand off to the runtime** — a runtime executes; evidence updates the graph.

The product-vision UX scenario:

> A team wants to add OAuth login. They describe the intent to their agent.
> The agent invokes the `helix` skill against existing governing artifacts.
> The skill produces an authority-ranked plan. The team reviews,
...
| align | Check alignment | Running the `helix` skill against an existing artifact set, getting findings, getting an authority-ranked plan | `/helix align` |
| plan | Identify planning gaps | Producing authority-ranked gaps with destination artifacts, deliverable shapes, next modes, and evidence | `/helix align` |
| evolve | Evolve a requirement | The OAuth-style scenario: a new requirement threads through PRD → feature spec → solution design → security architecture | `/helix evolve` |
...
| review | Fresh-eyes review | Reviewer agent inspects completed work against the artifacts that govern it; findings become runtime work items | `/helix review` |
| execute | Hand off to the runtime | Handoff of a planned bead to DDx; runtime evidence flows back and the artifact graph updates | `ddx bead execute` |
