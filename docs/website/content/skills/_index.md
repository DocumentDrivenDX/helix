...
package the artifact catalog, artifact authority hierarchy, and workflow modes
so an AI runtime can inspect a repository's Markdown artifacts and propose the
next safe plan. For the methodology behind the skill, see
...
{{< cards >}}
  {{< card link="#the-helix-routing-skill" title="The helix Routing Skill" subtitle="Resolve scope, choose a workflow mode, and plan authority-ranked artifact updates." icon="sparkles" >}}
{{< /cards >}}

### The helix Routing Skill

The core HELIX skill is `helix`. It reads an artifact set, resolves the owner
scope, compares artifacts via the authority hierarchy, chooses a workflow mode,
and returns a plan for restoring coherence. It is not an execution engine. It
...
{{< cards >}}
  ...
  {{< card link="../artifacts" title="HELIX Dogfood Artifacts" subtitle="A generated example corpus: HELIX applies its catalog to itself under docs/helix/." icon="document-text" >}}
